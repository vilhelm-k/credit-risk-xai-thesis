...
2. Concatenated into a single DataFrame
3. Converted to parquet format for efficient storage and loading
4. No filtering, transformations, or feature engineering applied at this stage
...
2. Filtered for non-financial corporation borrowing rates
3. Computed annual averages for short-term and long-term rates
...
```bash
# Place raw files in data/raw/ and data/external/, then run:
python3 credit_risk_xai/data/make_dataset.py   # Raw .dta → interim parquet
python3 credit_risk_xai/data/make_macro.py     # Raw CSVs → macro parquet
```
...
│   ├── external/                  <- SCB macro CSV files (not included, see above)
